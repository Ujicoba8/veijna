...
(function () {
  ...
  if (window.__hchLoaded) {
    console.log('[HCH] Already loaded');
    return;
  }
  ...
  style.textContent = `
    ...
    .hch-row.top {
      background: linear-gradient(135deg, #1a1400, #201800);
    ...
    .hch-mv {
      ...
      font-weight: 700; color: #ddd; flex: 1; letter-spacing: 0.05em;
    }
  ...
  panel.innerHTML = `
    <div class="hch-head" id="hch-head">
      ...
      <div class="hch-btns">
        <button class="hch-btn" id="hch-min" title="Minimize">−</button>
        <button class="hch-btn" id="hch-close" title="Close">✕</button>
      </div>
    </div>
    <div class="hch-body" id="hch-body">
      <div class="hch-status">
        <span class="hch-dot" id="hch-dot"></span>
        <span id="hch-status-txt">Connecting to server...</span>
      </div>
  ...
  document.getElementById('hch-close').onclick = () => {
    panel.remove();
    window.__hchLoaded = false;
  };
  ...
  function renderMoves(moves) {
    ...
    if (!moves || !moves.length) {
      el.innerHTML = '<div class="hch-empty">No moves found</div>';
      return;
    }
    el.innerHTML = moves.map((m, i) => {
      const sc = m.score;
      const scClass = sc.includes('Mate') || sc.includes('mate') ? 'mat'
        : parseFloat(sc) > 0.5 ? 'win'
        : parseFloat(sc) < -0.5 ? 'los' : 'eq';
      const uci = m.move;
      const notation = uci.slice(0, 2) + ' → ' + uci.slice(2, 4) + (uci[4] ? '=' + uci[4].toUpperCase() : '');
      return `
        <div class="hch-row ${i === 0 ? 'top' : ''}">
  ...
  function detectFen() {
    ...
    // Try chessboard.js
    const cb = document.querySelector('.board-b72b1, #board, .chessboard');
    if (cb) return fenFromCb(cb);

    // Try to find FEN in page source
  ...
  function fenFromCg(wrap) {
    try {
      ...
      pieces.forEach(p => {
        ...
        const t = style.match(/translate\((\d+(?:\.\d+)?)%,\s*(\d+(?:\.\d+)?)%\)/);
        if (!t) return;
  ...
  function fenFromCb(el) {
    try {
      const sqs = el.querySelectorAll('[data-square]');
      ...
      sqs.forEach(sq => {
        ...
        if (!pc) return;
        const m = pc.className.match(/([wb])([prnbqk])/i);
        if (!m) return;
        const fi = sqn.charCodeAt(0) - 97;
        const ri = 8 - parseInt(sqn[1]);
        board[ri][fi] = m[1].toLowerCase() + m[2].toLowerCase();
      });
      return boardToFen(board);
    } catch { return null; }
  }
  ...
  // ── Observer ───────────────────────────────────────────
  const target = document.querySelector('.cg-wrap, #board, [id*="board"], [class*="board"]') || document.body;
  ...
  console.log('[HCH] Bookmarklet loaded ✓');
})();
